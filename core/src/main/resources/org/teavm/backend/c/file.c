...
int64_t teavm_file_lastModified(char16_t* name, int32_t nameSize) {
    WCHAR* nativeName = teavm_file_convertPath(name, nameSize);
    FILETIME modified;
    BOOL status = GetFileTime(nativeName, NULL, NULL, &modified);
    free(nativeName);
    if (!status) {
        return 0;
    }
...
int32_t teavm_file_setLastModified(char16_t* name, int32_t nameSize, int64_t lastModified) {
    ...
    BOOL status = SetFileTime(nativeName, NULL, NULL, &modified);
    free(nativeName);
    return status;
}
...
int64_t teavm_file_open(char16_t* name, int32_t nameSize, int32_t mode) {
    int32_t readable = mode |= 1;
    int32_t writable = mode |= 2;
    int32_t append   = mode |= 4;

    DWORD creationDisposition = writable ? append ? OPEN_ALWAYS : CREATE_NEW : OPEN_EXISTING;
    ...
    return fileHandle != INVALID_HANDLE_VALUE ? (int64_t) fileHandle : 0;
}
